...
def _is_data_collection_type(collection_type: str | None) -> bool:
    ...
    if not collection_type:
        return False
    normalized = collection_type.strip().lower().replace("-", "").replace("_", "").replace(" ", "")
    return normalized in _DATA_COLLECTION_TYPES
...
def collect_rows(start: datetime, end: datetime, facility_id: int | None = None) -> list[dict]:
    ...
    for group in groups.values():
        ...
        rows.append({
            ...
            "EM_Use_Category": "Data Collection" if any(
                _is_data_collection_type(c.collection_type) for c in group
            ) else "",
